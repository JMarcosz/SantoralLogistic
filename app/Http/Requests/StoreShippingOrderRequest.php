...
class StoreShippingOrderRequest extends FormRequest
{
     ...
     * Set default values for nullable fields that have NOT NULL constraints in the database.
     */
    protected function prepareForValidation(): void
    {
        $this->merge([
            'total_amount' => $this->input('total_amount') ?? 0,
            'total_pieces' => $this->input('total_pieces') ?? 0,
            'total_weight_kg' => $this->input('total_weight_kg') ?? 0,
            'total_volume_cbm' => $this->input('total_volume_cbm') ?? 0,
        ]);
    }
    ...
    {
        return $this->user()->can('create', \App\Models\ShippingOrder::class);
    }
